...
static esp_err_t eth_set_mac_filter(void *h, const uint8_t *eth_mac, size_t mac_len, bool add)
{
    ...
    ESP_LOGI(TAG, "%s filter MAC: %02x:%02x:%02x:%02x:%02x:%02x", add ? "Add" : "Del", eth_mac[0], eth_mac[1],
             eth_mac[2], eth_mac[3], eth_mac[4], eth_mac[5]);
    if (add) {
...
static void eth_action_got_ip(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    ESP_LOGI(TAG, "eth_action_got_ip: %p, %p, %" PRIi32 ", %p, %p", netif_glue, base, event_id, event_data, *(esp_eth_handle_t *)event_data);
    if (netif_glue->base.netif == ip_event->esp_netif) {
        esp_netif_action_got_ip(ip_event->esp_netif, base, event_id, event_data);
        // Enable receive log for DM9051 after getting IP
        #define ETH_DM9051_CMD_ENABLE_RECEIVE_LOG  (ETH_CMD_CUSTOM_MAC_CMDS_OFFSET + 8)
        if (netif_glue->eth_driver) {
            esp_eth_ioctl(netif_glue->eth_driver, ETH_DM9051_CMD_ENABLE_RECEIVE_LOG, NULL);
        }
    }
}
