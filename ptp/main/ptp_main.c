...
// DM9051 PTP clock function stubs (for compilation)
#if defined(CONFIG_ETH_USE_ESP32_DM9051_PTP) || defined(ASSERT_DM9_PTP)
static inline int esp_dm9051_clock_gettime(clockid_t clock_id, struct timespec *tp) {
    return esp_eth_clock_gettime(clock_id, tp);
}

static inline int esp_dm9051_clock_set_target_time(clockid_t clock_id, struct timespec *tp) {
    return esp_eth_clock_set_target_time(clock_id, tp);
}

static inline int esp_dm9051_clock_register_target_cb(clockid_t clock_id,
                                                      ts_target_exceed_cb_from_isr_t ts_callback) {
    return esp_eth_clock_register_target_cb(clock_id, ts_callback);
}
#endif
#endif

static const char *TAG __attribute__((unused)) = "ptp_example";
...
void init_ethernet_and_netif(void)
{
    ...
    // Save the first eth_handle for PTP clock operations
    if (eth_port_cnt > 0) {
...
#if defined(CONFIG_ETH_USE_ESP32_EMAC) || defined(CONFIG_ETH_USE_ESP32_DM9051_PTP) || defined(ASSERT_DM9_PTP)
    // Initialize PTP clock subsystem
    if (s_eth_handle) {
...
ESP_LOGI(TAG, "DEBUG: Simulated while(1)");
...
while (1) {
    ...
    if (1) {
        ...
        while (1) {
            ...
            clock_source_valid_last = clock_source_valid;
            vTaskDelay(pdMS_TO_TICKS(100)); // 加入這行來餵狗並釋放 CPU
        }
    }
#else
    ESP_LOGI(TAG, "PTP is not supported on this Ethernet configuration. Only basic Ethernet functionality is available.");
    while (1) {
